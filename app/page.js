...
const ICON_MAP = {
  'atom': Atom, 'heart-pulse': HeartPulse, 'graduation-cap': GraduationCap,
  'book-open': BookOpen, 'refresh-cw': RefreshCw,
};
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-center gap-3">
        ...
        <div className="flex gap-1 flex-shrink-0">
          {announcements.map((_, i) => (
            <button key={i} onClick={() => setIdx(i)} className={`w-2 h-2 rounded-full transition-colors ${i === idx ? 'bg-royal-800' : 'bg-royal-800/30'}`} />
          ))}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-2xl shadow-2xl">
      <div className="relative" style={{ paddingBottom: '40%' }}>
        {banners.map((b, i) => (
          <div key={b.id} className={`absolute inset-0 transition-opacity duration-700 ${i === idx ? 'opacity-100' : 'opacity-0'}`}>
            <img src={b.image_url} alt={b.title} className="w-full h-full object-cover" />
            {(b.title || b.description) && (
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end p-8">
                <div>
                  {b.title && <h3 className="text-2xl md:text-3xl font-bold text-white mb-2">{b.title}</h3>}
                  {b.description && <p className="text-white/80 text-sm md:text-base">{b.description}</p>}
                </div>
              </div>
            )}
          </div>
...
export default function HomePage() {
  ...
  const navLinks = [
    ...
    { label: 'Notes', href: '#notes' }, { label: 'Contact', href: '#contact' },
  ];
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 md:h-[72px]">
            ...
            <a href="#home" className="flex items-center space-x-3">
              <img src={LOGO_URL} alt="RW Logo" className="w-10 h-10 md:w-11 md:h-11 rounded-full object-cover border-2 border-gold/40" />
              <div>
                <h1 className="text-lg md:text-xl font-bold text-white leading-tight">
                  RESULT WALLAH<sup className="text-[8px] text-gold-light ml-0.5">&trade;</sup>
                </h1>
            ...
            <div className="hidden lg:flex items-center space-x-7">
              {navLinks.map(l => (
                <a key={l.href} href={l.href} className="text-white/80 hover:text-gold-light transition-colors text-sm font-medium">{l.label}</a>
              ))}
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-royal-800 border-t border-white/10 pb-4">
            <div className="px-4 space-y-2 pt-2">
              {navLinks.map(l => (
                <a key={l.href} href={l.href} className="block text-white/80 hover:text-gold-light py-2 text-sm" onClick={() => setMobileMenuOpen(false)}>{l.label}</a>
              ))}
      ...
      {/* ═══ FOUNDER & DIRECTORS SECTION — First section below nav ═══ */}
      <section id="about" className="gradient-royal py-16 md:py-20">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {/* Founder — centre on mobile, first in grid */}
            {founder && (
              <div className="md:col-span-3 flex justify-center mb-4 md:mb-8">
                <div className="text-center max-w-sm">
                  ...
                  <Badge className="bg-gold/20 text-gold-light border-gold/30 text-xs md:text-sm px-4 py-1.5">
                    <Award className="w-4 h-4 mr-1.5" />
                    Founder - Vice City Mayor of Karad Nagar Palika
                  </Badge>
                  {founder.description && (
                    <p className="text-white/60 text-sm mt-4 leading-relaxed">{founder.description}</p>
                  )}
                </div>
              </div>
            )}
            ...
            {directors.map(d => (
              <div key={d.id} className="md:col-span-1 md:col-start-auto first:md:col-start-1 last:md:col-start-3 flex justify-center">
          ...
          {/* Additional Staff (if more than founder + 2 directors) */}
          {staff.length > 3 && (
            <div className="mt-14">
              <h3 className="text-xl font-bold text-white text-center mb-8">Our Expert Faculty</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto">
                {staff.slice(3).map(m => (
                  <div key={m.id} className="text-center">
                    <div className="w-20 h-20 mx-auto rounded-full border-2 border-white/20 overflow-hidden mb-2">
                      {m.photo_url ? <img src={m.photo_url} alt={m.name} className="w-full h-full object-cover" /> : <span className="flex items-center justify-center h-full text-xl font-bold text-gold-light">{m.name?.charAt(0)}</span>}
                    </div>
                    <h4 className="text-white font-semibold text-sm">{m.name}</h4>
                    <p className="text-white/50 text-xs">{m.designation}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="text-center lg:text-left">
              ...
              <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold text-white leading-tight mb-6">
                Transforming <span className="text-gradient-gold">Aspirations</span><br />into <span className="text-gradient-gold">Achievements</span>
              </h2>
              <p className="text-lg md:text-xl text-white/70 mb-8 max-w-lg mx-auto lg:mx-0">
                Expert coaching for JEE, NEET & MHT-CET with proven results. Limited batches of 50 students for personalized attention.
              </p>
              ...
              {results.map(r => (
                <Card key={r.id} className="hover-lift border-0 shadow-lg text-center overflow-hidden">
                  ...
                  <CardContent className="p-5">
                    <div className="w-24 h-24 mx-auto rounded-full bg-royal-100 border-2 border-gold flex items-center justify-center mb-3 overflow-hidden shadow-lg">
                      {r.photo_url ? <img src={r.photo_url} alt={r.student_name} className="w-full h-full object-cover" /> : <Trophy className="w-10 h-10 text-gold" />}
                    </div>
                    <h4 className="font-bold text-royal-800">{r.student_name}</h4>
                    <Badge className="mt-2 text-xs" style={{ backgroundColor: r.exam === 'JEE' ? '#3B82F6' : r.exam === 'NEET' ? '#10B981' : '#8B5CF6', color: 'white' }}>{r.exam}</Badge>
                    {r.percentile && <p className="text-gold-dark font-bold text-xl mt-2">{r.percentile}%ile</p>}
                    {r.marks && <p className="text-gray-500 text-sm">{r.marks} marks</p>}
                    {r.year && <p className="text-gray-400 text-xs mt-1">Batch {r.year}</p>}
                  </CardContent>
                </Card>
              ))}
            </div>
      ...
      {banners.length > 0 && (
        <section className="py-12 bg-white">
          <div className="max-w-6xl mx-auto px-4"><BannerCarousel banners={banners} /></div>
        </section>
      )}
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          <SectionTitle title="Director's Message" subtitle="A message from our leadership about our vision and commitment" />
                ...
                {settings?.directorMessage ? (
                  ...
                  <div className="space-y-6 text-gray-700">
                    ...
                    <ul className="space-y-2 list-none">
                      {['JEE Preparation (Main & Advanced)', 'NEET / NEET Repeater Preparation', 'MHT-CET Preparation', 'IX & X Foundation Courses'].map(p => (
                        <li key={p} className="flex items-center gap-2"><CheckCircle className="w-5 h-5 text-green-500 flex-shrink-0" /> {p}</li>
                      ))}
                    </ul>
                  </div>
                )}
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="space-y-2 mb-6">
                      {(c.features || []).slice(0, 4).map((f, i) => (
                        <div key={i} className="flex items-center gap-2 text-sm text-gray-700"><CheckCircle className="w-4 h-4 text-green-500 flex-shrink-0" />{f}</div>
                      ))}
                    </div>
        ...
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-10">
            {[{n:'Physics',i:Atom},{n:'Chemistry',i:FlaskConical},{n:'Mathematics',i:BookMarked},{n:'JEE',i:Zap},{n:'NEET',i:HeartPulse}].map(s => (
              <Card key={s.n} className="glass-card border-white/10 hover-lift cursor-pointer bg-white/5">
                <CardContent className="p-6 text-center">
                  <div className="w-12 h-12 mx-auto rounded-xl bg-gold/20 flex items-center justify-center mb-3"><s.i className="w-6 h-6 text-gold-light" /></div>
          ...
          <div className="glass-card border-white/10 rounded-2xl p-8 max-w-lg mx-auto">
            ...
            <p className="text-white/60 mb-6 text-sm">Register as a student to download study notes for free</p>
            <a href="/login"><Button className="bg-gold hover:bg-gold-dark text-royal-800 font-semibold px-8">Login / Register <ArrowRight className="w-4 h-4 ml-2" /></Button></a>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="flex justify-center gap-6 flex-wrap">
            <a href={contactInfo.facebook} target="_blank" rel="noopener noreferrer" className="group flex flex-col items-center gap-3 p-6 rounded-2xl hover:bg-blue-50 transition-colors">
              <div className="w-16 h-16 rounded-full bg-blue-600 flex items-center justify-center group-hover:scale-110 transition-transform"><Facebook className="w-8 h-8 text-white" /></div><span className="font-medium text-gray-700">Facebook</span>
            </a>
            <a href={contactInfo.instagram} target="_blank" rel="noopener noreferrer" className="group flex flex-col items-center gap-3 p-6 rounded-2xl hover:bg-pink-50 transition-colors">
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-purple-600 to-pink-500 flex items-center justify-center group-hover:scale-110 transition-transform"><Instagram className="w-8 h-8 text-white" /></div><span className="font-medium text-gray-700">Instagram</span>
            </a>
            <a href="https://wa.me/919156781002" target="_blank" rel="noopener noreferrer" className="group flex flex-col items-center gap-3 p-6 rounded-2xl hover:bg-green-50 transition-colors">
              <div className="w-16 h-16 rounded-full bg-green-500 flex items-center justify-center group-hover:scale-110 transition-transform"><MessageCircle className="w-8 h-8 text-white" /></div><span className="font-medium text-gray-700">WhatsApp</span>
            </a>
          </div>
        </div>
      </section>
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            <Card className="border-0 shadow-xl">
              <CardContent className="p-8">
                {contactSubmitted ? (
                  <div className="text-center py-12">
                    <div className="w-20 h-20 mx-auto rounded-full bg-green-100 flex items-center justify-center mb-4"><CheckCircle className="w-10 h-10 text-green-500" /></div>
                  ...
                  <form onSubmit={handleContactSubmit} className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div><label className="text-sm font-medium text-gray-700 mb-1 block">Mobile *</label><Input placeholder="Your mobile number" value={contactForm.mobile} onChange={e => setContactForm(p => ({ ...p, mobile: e.target.value }))} required /></div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid md:grid-cols-4 gap-8 mb-12">
            ...
            <div>
              ...
              <ul className="space-y-2">{navLinks.map(l => (<li key={l.href}><a href={l.href} className="text-white/60 hover:text-gold-light text-sm transition-colors flex items-center gap-1"><ChevronRight className="w-3 h-3" />{l.label}</a></li>))}</ul>
            </div>
            <div>
              <h4 className="text-white font-bold mb-4">Courses</h4>
              <ul className="space-y-2">{courses.slice(0, 5).map(c => (<li key={c.id}><a href="#courses" className="text-white/60 hover:text-gold-light text-sm transition-colors flex items-center gap-1"><ChevronRight className="w-3 h-3" />{c.name}</a></li>))}</ul>
            </div>
          </div>
      ...
      {/* ═══ FLOATING WHATSAPP ═══ */}
      <a href="https://wa.me/919156781002" target="_blank" rel="noopener noreferrer" className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-green-500 rounded-full flex items-center justify-center shadow-lg hover:bg-green-600 transition-all hover:scale-110 animate-pulse-gold">
